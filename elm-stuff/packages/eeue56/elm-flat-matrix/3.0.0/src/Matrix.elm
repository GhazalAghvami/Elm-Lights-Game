...
getColumn i matrix =
    let
        ...
        indices =
            List.map (\x -> x * width + i) [0..(height - 1)]
    in
        if i >= width then
            Nothing
        else
...
concatHorizontal a b =
    ...
    in
        if snd a.size /= snd b.size then
            Nothing
        else
            Just <|
                { a
                    | size = ( finalWidth, snd a.size )
                    , data =
                        List.foldl
...
concatVertical a b =
    if fst a.size /= fst b.size then
        Nothing
    else
        Just <| { a | size = ( fst a.size, snd a.size + snd b.size ), data = Array.append a.data b.data }
