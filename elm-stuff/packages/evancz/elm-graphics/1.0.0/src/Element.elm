...
-- for custom Elements implemented in JS, see collage for example
-- IMAGES
...
flow dir es =
    let
        ...
        hs =
            List.map heightOf es

        maxOrZero list =
            Maybe.withDefault 0 (List.maximum list)

        newFlow w h =
            newElement w h (Flow dir es)
    in
        if es == [] then
            empty
        else
            case dir of
                DUp ->
                    newFlow (maxOrZero ws) (List.sum hs)

                DDown ->
                    newFlow (maxOrZero ws) (List.sum hs)

                DLeft ->
                    newFlow (List.sum ws) (maxOrZero hs)

                DRight ->
                    newFlow (List.sum ws) (maxOrZero hs)

                DIn ->
                    newFlow (maxOrZero ws) (maxOrZero hs)
